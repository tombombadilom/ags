...
class LocalaiMessage extends Service {
    static {
...
class LocalAaiService extends Service {
    static {
        Service.register(this, {
            'initialized': [],
            'clear': [],
    ...
    addMessage(role, message) {
        this._messages.push(new LocalAaiMessage(role, message));
        this.emit('newMsg', this._messages.length - 1);
    }

    send(msg) {
        this._messages.push(new LocalAaiMessage('user', msg));
        this.emit('newMsg', this._messages.length - 1);
        const aiResponse = new LocalAaiMessage('model', 'thinking...', true, false)
...
export default new LocalAaiService();
